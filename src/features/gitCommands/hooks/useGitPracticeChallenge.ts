...
export const useGitPracticeChallenge = (): GitPracticeChallengeAPI => {
  ...
  const updateGitHubAfterPush = useCallback(
    (_remoteName: string, branchName: string, headCommit: Commit) => {
  ...
  // --- Funções Auxiliares ---
  const getBranchHeadCommit = useCallback(
    (branchName: string): Commit | undefined => {
  ...
  const isAncestor = useCallback(
    (commitIdToCheck: string, potentialAncestorId: string): boolean => {
  ...
  const handleShellCommand = useCallback(
    (mainCommand: string, commandParts: string[]): string => {
  ...
  const executeGitStatus = useCallback((): string => {
    ...
    if (workingDirectoryFiles.length > 0) {
      ...
      workingDirectoryFiles.forEach((file) => {
        statusMessage += `\t${file}\n`;
      });
      statusMessage += '\n';
    }
  ...
  const executeGitAdd = useCallback(
    (args: string[]): string => {
      ...
      if (fileArg === '.') {
        ...
        if (filesToRemoveFromWd.length > 0) {
          setWorkingDirectoryFiles((prevWDF) =>
            prevWDF.filter((f) => !filesToRemoveFromWd.includes(f))
          );
        }
        return '';
      } else if (fileArg) {
        if (currentDirContent[fileArg] && currentDirContent[fileArg].type === 'file') {
          const relativePathParts = currentPath.slice(gitRepoPath.length);
          const fullRelativePath = [...relativePathParts, fileArg]
            .filter((p) => p && p !== '~')
            .join('/');
  ...
  const executeGitCommit = useCallback(
    (commandPartsWithGit: string[]): string => {
  ...
  const executeGitLog = useCallback((): string => {
    ...
    commitsToDisplay.forEach((commit) => {
      ...
      let branchRefs = allBranches
        .filter((b) => getBranchHeadCommit(b)?.id === commit.id)
  ...
  const executeGitBranch = useCallback(
    (args: string[]): string => {
      ...
      if (!newBranchName) {
        ...
        if (
          newBranchName.includes(' ') ||
          !/^[a-zA-Z0-9_\-./]+$/.test(newBranchName) ||
          newBranchName.endsWith('/') ||
          newBranchName.includes('//') ||
          newBranchName.endsWith('.lock')
        ) {
        ...
        setBranchDetails((prev) =>
          new Map(prev).set(newBranchName, {
  ...
  const executeGitCheckout = useCallback(
    (args: string[]): string => {
      const arg1 = args[0];
      const arg2 = args[1];
      ...
      if (arg1 === '-b') {
        ...
        setBranchDetails((prev) =>
          new Map(prev).set(newBranchName, {
            parentBranch: currentBranch,
            parentCommitId: headCommitOnCurrentBranchCheckout
              ? headCommitOnCurrentBranchCheckout.id
              : null,
          })
        );
  ...
  const executeGitPush = useCallback(
    (args: string[]): string => {
      if (!isRepoInitialized)
        return 'fatal: not a git repository (or any of the parent directories): .git';
  ...
  const executeGitMerge = useCallback(
    (args: string[]): string => {
      if (!isRepoInitialized)
        return 'fatal: not a git repository (or any of the parent directories): .git';
    ...
    [isRepoInitialized, currentBranch, allBranches, getBranchHeadCommit, isAncestor, setCommits]
  );

  // Novas funções para GitHub

  const executeGitRemote = useCallback(
    (args: string[]): string => {
      if (!isRepoInitialized)
        return 'fatal: not a git repository (or any of the parent directories): .git';
  ...
  const executeGitPull = useCallback(
    (args: string[]): string => {
      if (!isRepoInitialized)
        return 'fatal: not a git repository (or any of the parent directories): .git';
  ...
  const executeGitClone = useCallback(
    (args: string[]): string => {
  ...
  const handleGitCommand = useCallback(
    (gitSubCommand: string | undefined, commandParts: string[]): string => {
      ...
      switch (gitSubCommand) {
        ...
        case 'clone':
          return executeGitClone(remainingArgs);
        case 'status':
        ...
        case 'merge':
          return executeGitMerge(remainingArgs);
        case 'remote':
          return executeGitRemote(remainingArgs);
        case 'pull':
          return executeGitPull(remainingArgs);
        default:
          if (gitSubCommand)
  ...
  const generateMermaidDiagramDefinition = useCallback((): string => {
    ...
    const chronologicalCommits = [
      ...(actualCommits.length > 0 ? actualCommits : commits),
    ].reverse();
    ...
    if (
      ...
      (chronologicalCommits.length === 1 && chronologicalCommits[0].id === 'Initial')
    ) {
    ...
    chronologicalCommits.forEach((commit) => {
      ...
      if (
        commit.branch !== mermaidCheckoutBranch &&
        !declaredMermaidBranches.has(commit.branch) &&
        commitBranchMeta
      ) {
        const safeParentBranchName = commitBranchMeta.parentBranch
          ? escapeMermaidString(commitBranchMeta.parentBranch)
          : null;
  ...
  const createRepository = useCallback(
    (repoName: string, repoDescription: string, isPrivate: boolean): void => {
